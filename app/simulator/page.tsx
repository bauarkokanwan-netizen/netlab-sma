...
function DeviceNode({ data }: any) {
  ...
  return (
    <div className="relative min-w-[170px] rounded-2xl border-2 border-slate-700 bg-white px-4 py-3 text-center shadow-sm">
      ...
      <div className="mt-2 flex items-center justify-center gap-2 text-[11px] font-semibold text-slate-700">
...
export default function SimulatorPage() {
  ...
  function runPing() {
    ...
    if (!from || !to || fromId === toId) {
      setResult("Tambahkan minimal 2 PC/Printer, lalu pilih asal dan tujuan yang berbeda.");
      return;
    }
    ...
    if (!networkConnected(from.id, to.id, typedEdges, typedNodes)) {
    ...
    if (getSubnet24(from.data.ip) !== getSubnet24(to.data.ip)) {
      setResult(`❌ Ping gagal: subnet berbeda. ${from.data.ip} dan ${to.data.ip} tidak satu jaringan /24.`);
      return;
    }
      ...
      <header className="border-b bg-white px-5 py-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500">Canvas kosong. Pilih kabel, tambah perangkat, isi IP, lalu test ping.</p>
          </div>
      ...
      <section className="grid flex-1 grid-cols-1 gap-4 p-4 lg:grid-cols-[1fr_390px]">
        ...
        <aside className="space-y-4">
          <div className="rounded-3xl bg-white p-5 shadow">
            ...
            <select value={fromId} onChange={(event) => setFromId(event.target.value)} className="mt-1 w-full rounded-xl border p-2">
              <option value="">Pilih perangkat</option>
              {endDeviceNodes.map((node) => <option key={node.id} value={node.id}>{node.data.label}</option>)}
            </select>

            <label className="mt-3 block text-sm font-semibold">Ke Perangkat</label>
            <select value={toId} onChange={(event) => setToId(event.target.value)} className="mt-1 w-full rounded-xl border p-2">
              <option value="">Pilih perangkat</option>
              {endDeviceNodes.map((node) => <option key={node.id} value={node.id}>{node.data.label}</option>)}
            </select>
          ...
          <div className="rounded-3xl bg-white p-5 shadow">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex items-center gap-2">{typedEdges.length >= 2 ? <CheckCircle2 className="h-4 w-4 text-green-600" /> : <XCircle className="h-4 w-4 text-red-600" />} Minimal 2 kabel</div>
              <div className="flex items-center gap-2">{invalidCableCount === 0 ? <CheckCircle2 className="h-4 w-4 text-green-600" /> : <XCircle className="h-4 w-4 text-red-600" />} Kabel benar: {validCableCount}/{typedEdges.length}</div>
          ...
          <div className="rounded-3xl bg-white p-5 shadow">
            ...
            {selectedNode ? (
              <div className="space-y-3">
                ...
                <input value={selectedNode.data.label} onChange={(event) => updateSelected("label", event.target.value)} className="w-full rounded-xl border p-2" />
                {canConfigureIp && (
                  <>
                    <label className="block text-sm font-semibold">IP Address</label>
                    <input value={selectedNode.data.ip} onChange={(event) => updateSelected("ip", event.target.value)} className="w-full rounded-xl border p-2" placeholder="contoh: 192.168.1.2" />
                    <label className="block text-sm font-semibold">Subnet Mask</label>
                    <input value={selectedNode.data.mask} onChange={(event) => updateSelected("mask", event.target.value)} className="w-full rounded-xl border p-2" placeholder="contoh: 255.255.255.0" />
                    <label className="block text-sm font-semibold">Gateway</label>
                    <input value={selectedNode.data.gateway} onChange={(event) => updateSelected("gateway", event.target.value)} className="w-full rounded-xl border p-2" placeholder="contoh: 192.168.1.1" />
                  </>
                )}
              </div>
            ) : (
          ...
          <div className="rounded-3xl bg-white p-5 text-sm shadow">
            ...
            <div className="mt-2 flex flex-wrap gap-3">
              <div className="flex items-center gap-2"><span className="h-3 w-3 rounded-full bg-green-500" /> Connected</div>
              <div className="flex items-center gap-2"><span className="h-3 w-3 rounded-full bg-yellow-400" /> Waiting / kabel salah</div>
              <div className="flex items-center gap-2"><span className="h-3 w-3 rounded-full bg-red-500" /> Disconnected</div>
            </div>
          </div>
        </aside>
      </section>
